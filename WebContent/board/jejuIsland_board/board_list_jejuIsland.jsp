...
$(function(){
	...
	$(".click-table > tbody > tr").click(function(){
		...
		<c:url var="path" value="/board_detail_jejuIsland.nanum">
		...
		alert("로그인 해주세요");
			...
			var theURL = path + "/board_list_jejuIsland.nanum";						
			...
			function closeSignUpWindow(){	
				var path = getContextPath();						
				var theURL = path + "/signUp_jejuIsland.nanum";						
				// 호출 한 부모 페이지에서 URL 호출						
				opener.window.location = theURL;						
				// 호출 한 뒤 현재 팝업 창 닫기 이벤트						
				close();						
...
<div class="col-sm-8">
	...
	<div class="kboard-header">
		...
		<form id="kboard-search-form" method="get" action="/demo/kboard-ocean-franchise-skin/?mod=list&pageid=1">
			...
			<a class="kboard-branch-button" href="<%=path%>/board_list_seoul.nanum" title="서울">서울</a>
				...
				<a class="kboard-branch-button active" href="<%=path%>/board_list_jejuIsland.nanum" title="제주도">제주도</a>
			</div>
		</form>
	</div>
	</div>
	...
	<table class="table table-hover click-table">
		<thead>
		<caption align="right">제주도 게시판</caption>
			<tr>
	...
	<c:url var="path" value="/board_write_jejuIsland.nanum">		
	</c:url>
	...
	<div style="text-align:center">
		<ul class="pagination">
			<c:url var="path" value="/board_list_jejuIsland.nanum">
