 ...
 */
@WebServlet("/BusanBoardDetailServlet")
public class BusanBoardDetailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public BusanBoardDetailServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			// 게시글 정보를 가져온다.
			BoardBean board_bean = BoardBusanDAO.getBoardContent(board_idx);
			...
			if(board_bean.getBoard_writer_idx() != login_bean.getU_idx()){
				BoardBusanDAO.addReadCnt(board_idx);
			}
			
			// 댓글 정보를 가져온다.
			ArrayList<ReplyBean> reply_list = BoardBusanDAO.getReplyList(board_idx);
			request.setAttribute("reply_list", reply_list);
			
			String site = "/board/busan_board/board_detail_busan.jsp";
