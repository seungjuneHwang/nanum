 ...
 */
@WebServlet("/GyeongbukBoardDetailServlet")
public class GyeongbukBoardDetailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GyeongbukBoardDetailServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			// 게시글 정보를 가져온다.
			BoardBean board_bean = BoardGyeongbukDAO.getBoardContent(board_idx);
			...
			if(board_bean.getBoard_writer_idx() != login_bean.getU_idx()){
				BoardGyeongbukDAO.addReadCnt(board_idx);
			}
			
			// 댓글 정보를 가져온다.
			ArrayList<ReplyBean> reply_list = BoardGyeongbukDAO.getReplyList(board_idx);
			request.setAttribute("reply_list", reply_list);
			
			String site = "/board/gyeongbuk_board/board_detail_gyeongbuk.jsp";
