 ...
 */
@WebServlet("/GangwondoBoardDetailServlet")
public class GangwondoBoardDetailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GangwondoBoardDetailServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(board_bean.getBoard_writer_idx() != login_bean.getU_idx()){
				BoardGangwondoDAO.addReadCnt(board_idx);
			}
			
			// 댓글 정보를 가져온다.
			ArrayList<ReplyBean> reply_list = BoardGangwondoDAO.getReplyList(board_idx);
			request.setAttribute("reply_list", reply_list);
			
			String site = "/board/gangwondo_board/board_detail_gangwondo.jsp";
