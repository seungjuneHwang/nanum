 ...
 */
@WebServlet("/SeoulBoardDetailServlet")
public class SeoulBoardDetailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SeoulBoardDetailServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			if(board_bean.getBoard_writer_idx() != login_bean.getU_idx()){
				BoardSeoulDAO.addReadCnt(board_idx);
			}
			
			// 댓글 정보를 가져온다.
			ArrayList<ReplyBean> reply_list = BoardSeoulDAO.getReplyList(board_idx);
			request.setAttribute("reply_list", reply_list);
			
			String site = "/board/seoul_board/board_detail_seoul.jsp";
