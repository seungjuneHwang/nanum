 ...
 */
@WebServlet("/GyeonggiBoardDetailServlet")
public class GyeonggiBoardDetailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public GyeonggiBoardDetailServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			// 게시글 정보를 가져온다.
			BoardBean board_bean = BoardGyeonggiDAO.getBoardContent(board_idx);
			...
			if(board_bean.getBoard_writer_idx() != login_bean.getU_idx()){
				BoardGyeonggiDAO.addReadCnt(board_idx);
			}
			
			// 댓글 정보를 가져온다.
			ArrayList<ReplyBean> reply_list = BoardGyeonggiDAO.getReplyList(board_idx);
			request.setAttribute("reply_list", reply_list);
			
			String site = "/board/gyeonggi_board/board_detail_gyeonggi.jsp";
